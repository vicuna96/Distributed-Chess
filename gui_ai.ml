...
let parse_click x y sq_dim =
  ...
  if col >= 1 && col <= 8 && row >= 1 && row <= 8 then
    Square (pos_correction col,pos_correction row)
...
(*       let () = sound 1500 1000 in *)
      ...
      if click_status then
            ...
            begin
              ...
              then
                ...
                let (st',mv'),flag =
                  ...
                  |PromoteKnight -> (Board.move st mv piece (Some "N")),true
                  ...
                  |_ -> (st,mv),false
                in try
                display_board st' sq_opt [] sq_dim;
                let oppo_move = AI.compute_next st' (Board.get_turn_color st') 2 in
                game_loop (oppo_move|>fst) (false,None,[],[])
                with
                | Not_found -> game_loop st' (false,None,[],[])
                )
              else
              (
                let () = sound 800 1000 in
                let st',mv' = Board.move st mv piece None in try
                display_board st' sq_opt [] sq_dim;
                let oppo_move = AI.compute_next st' (Board.get_turn_color st') 2 in
                game_loop (oppo_move|>fst) (false, None, [], [])
                with
        ...
        |_ -> let () = sound 800 1000 in game_loop st (false, None , [], [])
      else
        match click_type with
        |Square (c,r) ->
          if (Board.is_empty (c,r) st) then
            let () = sound 800 1000 in
